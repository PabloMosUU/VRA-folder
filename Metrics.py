...
class Metrics:
    ...
    def calc_auk(self):
        ...
        for i, prob in enumerate(self.probabilities_set[:-1]):
            ...
            auk += bottom
            #print('AUK is: ',str(auk))
            if self.integral is 'trapezoid':
                top = (y_dist * x_dist)/2
                auk += top
                #print('Trapezoid is calculated. Top is area: ',str(top))
            elif self.integral is 'max':
                top = (y_dist * x_dist)
                auk += top
                #print('Max is calculated. Top is area: ',str(top))
            else:
                continue
            #print('AUK after addition is: ',str(auk))
            
        return auk
       
       
    #Calculate roc-auc
    def calc_roc_auc(self, integral='trapezoid'):
        assert integral in ['trapezoid','max','min'], '"'+str(integral)+'"'+ ' is not a valid integral value. Choose between "trapezoid", "min" or "max"'
        roc_auc = 0
        fpr_list = self.fpr_curve()
    ...
    def calc_pr_auc(self, integral='trapezoid'):
        assert integral in ['trapezoid','max','min'], '"'+str(integral)+'"'+ ' is not a valid integral value. Choose between "trapezoid", "min" or "max"'
        pr_auc = 0
        ...
        for i, prob in enumerate(self.probabilities_set[:-1]):
            ...
            if self.integral is 'trapezoid':
                ...
                continue
        return pr_auc
            #The code below seems unnecessary now that 
        '''
            if step is False:                    
                top = (y_dist * x_dist)/2
                pr_auc += top
